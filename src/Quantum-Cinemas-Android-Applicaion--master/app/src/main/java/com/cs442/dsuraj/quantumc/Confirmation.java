...
public class Confirmation extends Activity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        final Bundle seat_info = intent.getBundleExtra("seat_information");
        ...
        final ArrayList<String> seating = seat_info.getStringArrayList("seats");
        ArrayList<String> total = seat_info.getStringArrayList("total");
        String movie_name = null;
        System.out.print(movie_id);
        final String seat_number = total.get(0);

        System.out.println("The seat number value is" + seat_number);


        total_price = Integer.parseInt(seat_number) * 45000;

        price_final = Integer.toString(total_price)+ " VND";
        Cursor movie = db.getMoviename(sql, movie_id);

        while (movie.moveToNext()) {
            movie_name ="Phim :"+movie.getString(0);
...

        {
            public void onClick(View v) {
                if (!isValidEmaillId(email.getText().toString().trim())||(phone.getText().toString().length() != 10)) {
                    ...
                    order.setText("Thanh toán (" + arr.get(4) + ")");
                    ...
                    order.setOnClickListener(new View.OnClickListener() {
                        public void onClick(View v) {
                            ...
                            if ((namecard.getText().toString().isEmpty()) || (creditcard.getText().toString().isEmpty()) || (cvvno.getText().toString().isEmpty()) || (valid.getText().toString().isEmpty())) {
                                ...
                                if (db.insertmoviebooked(Integer.parseInt(movie_id), seats.getText().toString(), theatre1, time1, date1,total_price, Email, phoneno)) {
